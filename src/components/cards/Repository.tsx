...
export default function Repository({
  ...
  return (
    <Link
      href={repo.html_url}
      target="_blank"
      className="flex h-20 w-full cursor-pointer items-center justify-between rounded-lg border px-2 drop-shadow-xl transition-colors duration-300 hover:bg-secondary/80 dark:bg-primary dark:hover:bg-primary/70 xl:h-16"
    >
      <div className="flex flex-row items-center gap-2">
        <div className="flex flex-col">
          <div className="flex w-full flex-row items-center gap-1 font-semibold">
            ...
            {repo.archived && (
              <span className="hidden flex-row items-center gap-1 rounded-full border border-zinc-600/40 bg-accent px-2 text-[12px] text-muted drop-shadow-xl xl:flex">
